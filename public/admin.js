...
const salesFilters = document.querySelector("#salesFilters");
const salesSummary = document.querySelector("#salesSummary");
const salesList = document.querySelector("#salesList");
const salesMessage = document.querySelector("#salesMessage");
...
async function refreshAll() {
  await Promise.all([loadDashboard(), refreshAgenda(), loadClients(), loadFinance(), loadSales()]);
}
...
async function loadSales() {
  ...
  const filters = new URLSearchParams(new FormData(salesFilters));
  salesMessage.textContent = "Atualizando vendas...";
  try {
    const payload = await api(`/api/admin/sales?${filters.toString()}`);
    const summary = payload.summary;
    salesSummary.innerHTML = [
      metricCard("Realizado", summary.realized_label, `${summary.completed_count} venda(s) concluída(s)`),
      metricCard("Previsto", summary.forecast_label, "Pendentes + confirmadas"),
      metricCard("Ticket médio", summary.average_ticket_label, "Média das vendas concluídas"),
      metricCard("Origens", `${summary.site_count} site · ${summary.admin_count} manual`, `${summary.records} registro(s) no período`),
    ].join("");
    salesList.innerHTML = payload.sales.length ? `
      <table class="ops-table">
        <thead><tr><th>Data</th><th>Cliente</th><th>Serviço</th><th>Valor</th><th>Status</th><th>Origem</th></tr></thead>
        <tbody>${payload.sales.map((sale) => `
          <tr>
    ...
    ` : `<p class="ops-empty">Nenhuma venda encontrada nesse período.</p>`;
    salesMessage.textContent = "";
  } catch (error) {
    salesMessage.textContent = error.message || "Não foi possível carregar as vendas.";
  }
}
...
async function loadAdminServices() {
  ...
  const salesServiceFilter = salesFilters?.querySelector("select[name='service_id']");
  if (salesServiceFilter) salesServiceFilter.innerHTML = `<option value="">Todos os serviços</option>${serviceOptions("")}`;
  refreshManualAppointmentOptions();
...
async function bootAdmin() {
  ...
  if (salesFilters) {
    salesFilters.querySelector("input[name='date_from']").value = `${todayIso().slice(0, 8)}01`;
    salesFilters.querySelector("input[name='date_to']").value = todayIso();
  }
  currentCalendarMonth = adminDate.value.slice(0, 7);
...
manualAppointmentForm?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await Promise.all([loadClients(), refreshAgenda(), loadDashboard(), loadFinance(), loadSales()]);
  } catch (error) {
